...
function tick(dialerId) {
  ...
  const maxConcurrent = Math.max(1, parseInt(dialerConfig.maxConcurrentCalls, 10) || 1);
  if (countPendingCallsForDialer(dialerId) >= maxConcurrent) return;

  let minRowIndex = 1;
  let next = null;
  for (;;) {
    next = getNextNotCalledRow(meta.path, targetZip, minRowIndex);
    while (next && (isBlacklisted(next.row.phone) || isAddressBooked(next.row.address))) {
      ...
      console.log(`[scheduler] ${dialerId} row ${next.rowIndex} skipped (${reason})`);
      minRowIndex = next.rowIndex + 1;
      next = getNextNotCalledRow(meta.path, targetZip, minRowIndex);
    }
    if (!next) return;
    const extId = buildExternalId(dialerId, dialerConfig.spreadsheetId, next.rowIndex);
    const isPending = state.pendingCallPhoneNumber?.[extId];
    const startedAt = state.pendingCallStartedAt?.[extId];
    if (isPending && startedAt && Date.now() - startedAt > PENDING_CALL_TIMEOUT_MS) {
      try {
        updateRow(meta.path, next.rowIndex, { status: 'called', endedReason: 'call-failed-timeout' });
      } catch (e) {
        console.error(`[scheduler] ${dialerId} timeout updateRow:`, e.message);
      }
      const phone = next.row.phone != null ? String(next.row.phone).replace(/\D/g, '').slice(-10) : '';
      if (phone && addToBlacklist(phone)) {
        console.log(`[scheduler] ${dialerId} row ${next.rowIndex} added to blacklist (timeout, no webhook):`, phone);
      }
      updateState((s) => {
        if (s.pendingCallPhoneNumber) delete s.pendingCallPhoneNumber[extId];
        if (s.pendingCallStartedAt) delete s.pendingCallStartedAt[extId];
        return s;
      });
      console.log(`[scheduler] ${dialerId} row ${next.rowIndex} marked called (timeout, no webhook)`);
      minRowIndex = next.rowIndex + 1;
      continue;
    }
    if (isPending) {
  ...
  let leaveVoicemail = shouldLeaveVoicemail(dialerId);
  // Double-tap on: first attempt never leaves voicemail — hang up if VM so VAPI can report customer-did-not-answer/voicemail, then we retry in 30s and leave VM on second attempt only.
  if (dialerConfig.doubleTap) {
  ...
  console.log(`[scheduler] ${dialerId} placing call row ${rowIndex} to ${row.phone} (externalId ${externalId})${dialerConfig.doubleTap ? ' [first attempt, no VM]' : ''}`);

  // Mark as 'calling' immediately — prevents any other dialer sharing this spreadsheet from picking up the same row.
  try {
    updateRow(meta.path, rowIndex, { status: 'calling' });
  } catch (e) {
    console.error(`[scheduler] ${dialerId} failed to mark row ${rowIndex} as calling:`, e.message);
  }
  ...
  createCall({
    ...
    // Mark as 'called' with reason — never reset to 'not-called' on failure,
    // so other dialers don't immediately retry the same contact.
    try {
      updateRow(meta.path, rowIndex, { status: 'called', endedReason: 'call-placement-failed' });
    } catch (e) {
      console.error(`[scheduler] ${dialerId} failed to mark row ${rowIndex} as called after placement failure:`, e.message);
    }
    updateState((s) => {
...
function buildExternalId(dialerId, spreadsheetId, rowIndex) {
  // 'dialer1' → 'd1', 'dialer4' → 'd4', etc.
  const d = dialerId.replace('dialer', 'd');
  const short = String(spreadsheetId || '').slice(0, 8);
  return `${d}:${short}:${rowIndex}`;
}

function pendingPrefix(dialerId) {
  return dialerId.replace('dialer', 'd') + ':';
}

function countPendingCallsForDialer(dialerId) {
  const state = getState();
  const prefix = pendingPrefix(dialerId);
  return Object.keys(state.pendingCallPhoneNumber || {}).filter((k) => k.startsWith(prefix)).length;
}
...
function scheduleDoubleTapRetry(externalId) {
  ...
  setTimeout(() => {
    ...
    // Second attempt: leave voicemail if configured (e.g. 1 VM to every 1 call). First attempt already hung up on VM without leaving one.
    let voicemailMessage = '';
    if (dialerConfig?.voicemailMessage) {
      voicemailMessage = substituteVariables(spin(dialerConfig.voicemailMessage), variableValues);
    }

    console.log(`[scheduler] ${dialerId} double-tap retry row ${rowIndex} (externalId ${externalId})${voicemailMessage ? ' [with VM]' : ''}`);

    // Mark as 'calling' immediately before retry too — prevents regular tick or other dialers from grabbing it.
    try {
      updateRow(meta.path, rowIndex, { status: 'calling' });
    } catch (e) {
      console.error('[scheduler] double-tap retry failed to mark row as calling:', e.message);
    }
    ...
    createCall({
      ...
      console.error('Double-tap createCall failed:', err.message);
      try {
        updateRow(meta.path, rowIndex, { status: 'called', endedReason: 'call-placement-failed' });
      } catch (e) {
        console.error('[scheduler] double-tap retry failed to mark row as called:', e.message);
      }
    });

...
function tick(dialerId) {
  ...
  let next = getNextNotCalledRow(meta.path);
  while (next && isBlacklisted(next.row.phone)) {
    try {
      updateRow(meta.path, next.rowIndex, { status: 'blacklisted', endedReason: 'blacklisted' });
    } catch (e) {
      console.error(`[scheduler] ${dialerId} blacklist updateRow:`, e.message);
    }
    console.log(`[scheduler] ${dialerId} row ${next.rowIndex} skipped (blacklisted)`);
    next = getNextNotCalledRow(meta.path);
  }
  if (!next) return;

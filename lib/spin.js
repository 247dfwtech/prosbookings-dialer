...
/** Variable names are matched case-insensitively so {{firstName}}, {{FIRSTNAME}}, {{FirstName}} all work. */
function substituteVariables(text, vars) {
  ...
  const byLower = {};
  for (const k of Object.keys(map)) byLower[keyLower(k)] = map[k];
  const getVal = (key) => map[key] ?? map[key.replace(/_([a-z])/g, (_, c) => c.toUpperCase())] ?? byLower[keyLower(key)] ?? '';

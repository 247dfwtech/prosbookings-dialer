...
function ensureBookedFile() {
  ...
  if (!fs.existsSync(BOOKED_PATH)) {
    ...
    const ws = XLSX.utils.aoa_to_sheet([['First Name', 'Last Name', 'Address', 'Phone', 'Transcript', 'Audio URL']]);
...
function addBooking({ firstName, lastName, address, phone, transcript, audioUrl }) {
  ensureBookedFile();
  try {
    const wb = XLSX.readFile(BOOKED_PATH);
    const firstSheet = wb.SheetNames[0];
    ...
    // If existing file has only 5 columns, upgrade header to include Audio URL
    if (data.length > 0 && data[0].length === 5) {
      data[0].push('Audio URL');
    }

    // Add new row: First Name, Last Name, Address, Phone, Transcript, Audio URL
    const newRow = [
...
module.exports = { isAddressBooked, addBooking, clearBooked, normalizeAddress };

...
const DEFAULT_STATE = {
  dialers: {
    dialer1: dialerStateDefaults(),
    dialer2: dialerStateDefaults(),
    dialer3: dialerStateDefaults(),
  },
  doubleTapRetry: {},
  retryScheduledFor: {},
...
function getConfig() {
  ...
  config.dialers = config.dialers || {};
  // Ensure defaults for all dialers (dynamic — not just 1-3)
  for (const id of Object.keys(config.dialers)) {
    const d = config.dialers[id];
    d.startTime = d.startTime ?? '';
    d.endTime = d.endTime ?? '';
    d.daysOfWeek = d.daysOfWeek || [1, 2, 3, 4, 5];
  }
  return config;
}
...
function getState() {
  ...
  state.retryScheduledFor = state.retryScheduledFor || {};
  // Ensure defaults for all dialers in state (dynamic)
  for (const id of Object.keys(state.dialers)) {
...
/**
 * Ensure a dialerId exists in both config and state with defaults.
 * Called when a new subuser is created.
 */
function ensureDialerInConfig(dialerId) {
  updateConfig((c) => {
    if (!c.dialers[dialerId]) {
      c.dialers[dialerId] = dialerDefaults();
      console.log(`[store] Created config entry for ${dialerId}`);
    }
    return c;
  });
  updateState((s) => {
    if (!s.dialers[dialerId]) {
      s.dialers[dialerId] = dialerStateDefaults();
      console.log(`[store] Created state entry for ${dialerId}`);
    }
    return s;
  });
}
